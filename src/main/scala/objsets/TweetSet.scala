...
abstract class TweetSet extends TweetSetInterface {
  ...
  def filter(p: Tweet => Boolean): TweetSet = {
    val newTweetSet = new Empty
    filterAcc(p, newTweetSet)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    //right.union(left.union(that.incl(elem)).incl(elem))
  }


  def mostRetweeted: Tweet = {

    def maxFromTwoTweets(firstTw: Tweet, secondTw: Tweet): Tweet = {
      if (firstTw.retweets > secondTw.retweets) firstTw else secondTw
    }

    (left, right) match {
      case (empty: Empty, empty2: Empty) => elem
      case (first: NonEmpty, second: Empty) => maxFromTwoTweets(elem, first.mostRetweeted)
      case (first: Empty, second: NonEmpty) => maxFromTwoTweets(elem, second.mostRetweeted)
